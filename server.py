...
class Controller(object):
    ...
    def __init__(self, app):
        ...
        self.app = app
        self.trans_id = '-'
        self.logger = get_logger(None,'proxy-server')
    ...
    def account_info(self, account, autocreate=False):
        ...
        if self.app.memcache:
            ...
            if not isinstance(cache_value, dict):
                ...
                bytes_used = cache_value['bytes_used']
            if result_code == 200:
                return partition, nodes, container_count, bytes_used
            ...
            try:
                ...
                with Timeout(self.app.node_timeout):
                    ...
                    if 200 <= resp.status <= 299:
                        ...
                        bytes_used = int(
                            resp.getheader('x-account-bytes-used') or 0)
                        break
        ...
        if self.app.memcache and result_code in (200, 404):
            ...
            self.app.memcache.set(cache_key,
                {'status': result_code, 'container_count': container_count, 'bytes_used': bytes_used},
    ...
    def _make_app_iter(self, node, source, response,tenant_ID = '-'):
        """
        ...
        try:
            try:
                ...
                self.app.logger.warn(_('Client disconnected on read . response.bytes_transferred = %r KB . ' % (response.bytes_transferred/1024)))
            except Exception:
            ...
            # Add by wangyangang
            self.app.logger.warn("In _make_app_iter() finall. response.bytes_transferred = %r " % response.bytes_transferred)
            self.app.add(tenant_ID,BONDOUT,response.bytes_transferred/1024)
            #End add by wangyangang
    ...
    def GETorHEAD_base(self, req, server_type, partition, nodes, path,
        ...
        if source:
            if req.method == 'GET' and source.status in (200, 206):
                ...
                self.logger.error("In GETorHEAD_base req.method = GET . source = %r " % source)
                res.app_iter = self._make_app_iter(node, source, res,req.headers.get('X-Tenant-Id','-'))
                self.logger.error("res.bytes_transferred = %r " % res.bytes_transferred)
                self.logger.error("---" *20)
                # See NOTE: swift_conn at top of file about this.
...
class ObjectController(Controller):
    ...
    def PUT(self, req):
        ...
        if self.app.max_space_per_account > 0 and \
            self.app.max_space_per_account <= self.account_info(self.account_name)[3]:
            aresp = HTTPForbidden(request=req)
            aresp.body = 'Reached space limit of %s' % \
                self.app.max_space_per_account
            return aresp
        ...
        #add by wangyangang for test
        self.logger.error("+++"*20)
        self.logger.error("In ObjectController PUT(), account_name = %r,  req.bytes_transfered = %r KB" %(self.account_name, req.bytes_transferred/1024 ))
        self.app.add(req.headers.get('X-Tenant-Id','-'),BONDIN,req.bytes_transferred/1024)
        self.logger.error("---"*20)
...
class ContainerController(Controller):
    ...
    def PUT(self, req):
        ...
        if len(self.container_name) > MAX_CONTAINER_NAME_LENGTH:
            ...
            return resp
        account_partition, accounts, container_count, bytes_used = \
...
class BaseApplication(object):
    ...
    def __init__(self, conf, memcache=None, logger=None, account_ring=None,
        ...
        self.max_containers_whitelist = [a.strip()
            ...
            if a.strip()]
        self.max_space_per_account = int(conf.get('max_space_per_account', 0))

        self.logger.error("+++" *20)
        self.logger.error("In BaseApplication __init__ ")
        self.logger.error("---" *20)


    #add by wangyangang for counts
        self.server_ID = int (conf.get('server_ID') or int(random()*10000))
        self.interval = int ( conf.get('flush_db_interval') or 60*60)
        self.db_host = conf.get('db_host', '10.12.28.100')
        self.db_user = conf.get('db_user', 'root')
        self.db_passwd = conf.get('db_passwd', '123456')
        
        self.stats_info = {}
        self.flush_db(self.stats_info )


    def flush_db(self,dicts):
        tmp_dicts = self.stats_info.copy()
        self.clear_all()
        self.flush_timer = timer.Timer(self.interval,self.flush_db,( tmp_dicts,))
        self.flush_timer.schedule()
        ...
        for key,value in tmp_dicts.items():
            sql = "insert into  swift_statistics(\
            ServerID, AddTime,AccountID,OperPUTCount, OperGETCount,OperHEADCount,OperPOSTCount,OperDELETECount,OperOTHERCount, BWIn,BWOut) \
            values(%d,'%s','%s',%d,%d,%d,%d,%d,%d,%d,%d)" \
            % (self.server_ID,strt,key,value[PUT],value[GET],value[HEAD],value[POST],value[DELETE],value[OTHER],value[BONDIN],value[BONDOUT])
            try:
                cursor.execute(sql)
            except Exception, e:
                self.logger.error("---" *20)
                self.logger.error("cursor.execute()  exception : %r" % e )
                self.logger.error("+++" *20)
                return
        ...
        if(time.time() - starttime) > 3600 :
            self.logger.error('data processing is more than 1 hour')
## if process time is more than 1 hour, then report warning and continue 
        #if  (time.time() - starttime) > 3600 :
        #    print 'data processing is more than 1 hour'    
        #time.sleep( (60 - time.localtime().tm_min)*60 - time.localtime().tm_sec )

    def add(self,accID,oper,bond_width = 0):
        if not self.stats_info.has_key(accID) :
            self.stats_info[accID] =  [0,0,0,0,0,0,0,0]
            if oper == BONDIN or oper == BONDOUT :
                self.stats_info[accID][oper] += bond_width
            else:
                self.stats_info[accID][oper] += 1
        else:
            if BONDIN == oper or BONDOUT == oper:
                self.stats_info[accID][oper] += bond_width
            else:
                self.stats_info[accID][oper] += 1
    ...
    def get_req_method(self,method):
        if method == 'GET':
            return GET
        elif method == 'PUT':
            return PUT
    ...
    #end add by wangyangang for counts
    ...
    def handle_request(self, req):
        ...
        try:
            ...
            self.logger.error("+++" *20)
            self.logger.error("In BaseApplication handle_request, req = %r \n,req.headers = %r\n req.environ = %r " % (req,req.headers,req.environ))
            self.logger.error("---" *20)
            ...
            self.logger.error("+++" *20)
            self.logger.error("CONTROLLER = %r. \n PATH_PARTS = %r " % (controller,path_parts))
            self.logger.error("---" *20)
            ...
            self.logger.error("+++" *20)
            self.logger.error("CONTROLLER after controller.  = %r.  " % (controller))
            self.logger.error("---" *20)
            ...
            if 'swift.authorize' in req.environ:
                ...
                # add by wangyangang test swift
                self.logger.error("+++" *20)
                self.logger.error("In handle_request swift.authorized, resp = %s" % resp )
                self.logger.error(" request.environ : %r" , req.environ['swift.authorize'])
                self.logger.error("---" *20)
                # end add by wangyangang test swift
            ...
            self.logger.error("+++" *20)
            self.logger.error("In handle_request , handler = %r . req.content_length = %r" % (handler,req.content_length) )
            self.add(req.headers.get('X-Tenant-Id','-'),self.get_req_method(req.environ.get('REQUEST_METHOD','-')))
            
            self.logger.error("          {AccountID: [GET, PUT, POST, DELETE, OTHER, BONDIN, BONDOUT")
            self.logger.error("Counts = %r" % self.stats_info)
            self.logger.error("---" *20)
